...
public class BasicDocument extends Document 
{
	...
	public int getNumWords()
	{
		...
		List<String> tokens = getTokens("[a-zA-Z]+"); // only count the words with "a-z" and "A-Z" EG : {pocket, joke, car} etc,..
	    return tokens.size();
	}
	...
	public int getNumSentences()
	{
        ...
        // if you need help.
		List<String> tokens = getTokens("[^.!?]+"); // add the words or sentences which are not contains ".!?" 
        return tokens.size();
        // sentence is nothing but ends with "." or "!" or "?"
	}
	...
	public int getNumSyllables()
	{
		...
		// and then call it here on each word.
		 ...
		 List<String> s = getTokens("[a-zA-Z]+");
		 int count = 0;
		 while(!s.isEmpty()) {
			 count = count + countSyllables(s.remove(s.size()-1));
		 }
	...
	public static void main(String[] args)
	{
		...
		testCase(new BasicDocument("Lorem ipsum dolor sit amet, qui ex choro quodsi moderatius, nam dolores explicari forensibus ad."),
		         32, 15, 1);
		//System.out.println(new BasicDocument("if you're single, you've never lost your spouse, and you're not the head of a household, then you pay taxes according to the following table.").getFleschScore());
	}
